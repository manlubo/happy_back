...
### 회원가입 시 기존 회원의 전화번호 제거가 반영되지 않는 문제

**[상황]**
- 신규 회원가입시 기존 회원이 동일한 번호를 사용하고 있는 경우
- 기존 회원의 휴대폰 번호를 제거 하고 신규 회원을 생성하도록 구현했으나, DB에 반영되지 않고 유니크제약에 걸려 회원가입이 실패

**[원인]**
- JPA의 flush 순서(INSERT => UPDATE => DELETE)에 의해 실패

**[해결]**
- 업데이트 이후 `flush()`호출을 통해 DB에 즉시 반영

### 회원가입 API 성능 저하

**[상황]**
- 회원가입 API 호출 시 평균 응답 시간이 약 1,800ms로 측정됨

**[원인]**
- 휴대폰 번호 중복 확인 과정에서 exists 조회 이후 동일 조건으로 회원 엔티티를 다시 조회하여 불필요한 중복 쿼리가 발생함
- 트랜잭션 중간에 flush()를 호출하여 영속성 컨텍스트 전체 동기화가 발생하고, 이로 인해 성능 저하가 발생함

**[해결]**
- JPQL bulk update를 사용하여 휴대폰 번호 중복 회원을 DB에 직접 업데이트하도록 개선
- `clearAutomatically = true` 옵션을 적용하여 영속성 컨텍스트와 DB 상태 간 불일치 문제를 방지
- 회원가입 API 응답 시간 1,800ms → 290ms로 단축(약 80% 감소)